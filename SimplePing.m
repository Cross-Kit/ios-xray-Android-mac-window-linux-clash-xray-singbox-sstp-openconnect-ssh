...
- (void)start {
    if ([self.hostName componentsSeparatedByString:@"."].count == 4) {
        in_addr_t ip = inet_addr((const char *)self.hostName.UTF8String);
        self.addressStyle = SimplePingAddressStyleICMPv4;
        struct sockaddr_in p;
        p.sin_addr.s_addr = ip;
        p.sin_family = AF_INET;
        self.hostAddress = [NSData dataWithBytes:&p length:sizeof(struct sockaddr_in)];
        [self startWithHostAddress];
        return;
    }
    Boolean             success;
